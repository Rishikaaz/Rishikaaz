# Hi there 👋, I'm Rishika Patel

> "Passionate about protecting the digital world, one byte at a time."

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/rishika-patel-854ba5289)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:riku192004@gmail.com)
[![Medium](https://img.shields.io/badge/Medium-12100E?style=for-the-badge&logo=medium&logoColor=white)](https://medium.com/@rishikaaz19)
[![TryHackMe](https://img.shields.io/badge/TryHackMe-212C42?style=for-the-badge&logo=tryhackme&logoColor=red)](https://tryhackme.com/p/Rishikaaz)

---

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white)
![BurpSuite](https://img.shields.io/badge/BurpSuite-FF6600?style=for-the-badge)
![Top 2% TryHackMe](https://img.shields.io/badge/TryHackMe-Top%202%25-brightgreen)

---

🎓 **B.Tech Computer Science Student | Cybersecurity Enthusiast **

- 🔒 Highly motivated Computer Science student with a strong passion for cybersecurity.
- 🏆 **CTF Ranks:** Ranked 355th/5947 in Industrial-Intrusion CTF (TryHackMe) | Top 2% on TryHackMe (Rank 38564).
- 🌱 **Currently learning:** Japanese (with an ongoing streak of 180 days on Duolingo).

---

## 🛠️ Tools & Languages

### 💻 Technical Skills

| Category | Languages / Frameworks / OS |
| :--- | :--- |
| **Programming** | C, C++, Python, Java, JavaScript, Bash, PL/SQL |
| **Database** | SQL, MySQL |
| **Web Dev** | Node.js, React.js, Express.js, Bootstrap, Tailwind, HTML, CSS |
| **Cloud/Tech** | Computer Networks, Network Security, OWASP, AWS (EC2, Lambda, S3, Elastic Beanstalk), Blockchain, Ethereum, Bitcoin |
| **OS** | Windows, Linux |

### 🛡️ Security Tools

| Tool Type | Tools Used |
| :--- | :--- |
| **Vulnerability Scanning** | Nessus, Nmap |
| **Interception/Analysis** | BurpSuite, Wireshark |
| **Exploitation/OS** | Metasploit, Kali |
| **Password Cracking** | Hashcat |

### 🗣️ Human Languages

- **English**
- **Hindi**
- **Awadhi**
- **Japanese** (Currently learning with an ongoing streak of 180 days on Duolingo)

---

## 💼 Experience & Certifications

* **Experience:**
    * Cybersecurity and Ethical Hacking at ApexPlanet Software Pvt Ltd (Nov 2025 - Present)
    * FOSS Contributor at Reskill (May-Aug 2025)
    * Cyber Security & Ethical Hacking Internship (Tamizhan Skills) (Jun-Jul 2025)
    * Green Skills with AI technologies (Edunet Foundation) (Jun-Jul 2025)
* **Certifications:**
    * Blockchain and its Applications (NPTEL, IIT Kharagpur)
    * Networking Basics (CISCO) and Introduction to Cybersecurity (CISCO)
    * Mastercard Cybersecurity & Deloitte Australia Cyber Job Simulations (Forage)

## 🚀 Projects & Accomplishments

* **CTF/Competitions:**
    * Ranked 355th/5947 in Industrial-Intrusion CTF (TryHackMe)
    * Completed Advent of Cyber 2025 (TryHackMe)
    * 7th rank in Shell n'Zen CTF, participated SECVIT'24 CTF
* **Hackathon/Events:**
    * Participant in "Cyber For Her" (DSCI), Flipkart Runway: Season 5
* **Open Source:** FOSS Contributor in Social Summer of Code - Season 4
* **AI/ML:** Worked on a Carbon Emission Project using ML, training a model to predict greenhouse gases.

---

<p align="center">
  <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="100" alt="Welcome GIF">
</p>

---

---

*Always learning. Always sharing. Let’s connect and secure the web together!*
